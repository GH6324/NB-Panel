  ...
  const cleanup = useCallback(() => {
    ...
    if (eventSourceRef.current && eventListenersRef.current.size > 0) {
      eventListenersRef.current.forEach((listener, eventType) => {
        try {
          ...
          // 静默处理移除失败的情况
          console.debug(`[NB SSE] 移除事件监听器失败: ${eventType}`);
        }
      });
    ...
    if (eventSourceRef.current) {
      try {
        ...
        // 静默处理关闭失败的情况
        console.debug("[NB SSE] 关闭EventSource失败");
      } finally {
    ...
    if (abortControllerRef.current) {
      try {
        ...
        // 静默处理abort失败的情况
        console.debug("[NB SSE] AbortController abort失败");
      } finally {
  ...
  const connect = useCallback(
    (endpoint: NPEndpoint) => {
      ...
      if (!endpoint || !endpoint.url || !endpoint.apiPath || !endpoint.apiKey) {
        console.error("[NB SSE] endpoint参数无效:", endpoint);
      ...
      try {
        ...
        const processEvent = (event: MessageEvent) => {
          console.log("[NB SSE] ========== 新消息开始 ==========");
          console.log("[NB SSE] 原始事件数据:", event.data);
          console.log("[NB SSE] 数据类型:", typeof event.data);

          try {
            // 首先尝试解析为JSON
            const data = JSON.parse(event.data);

            console.log("[NB SSE] JSON解析成功:", data);
            console.log("[NB SSE] 消息类型:", data.type);

            console.log("[NB SSE] 调用onMessage回调，传递数据:", data);
            onMessage(data);
            console.log("[NB SSE] onMessage回调调用完成");

            // 检查是否为连接确认消息
            if (data.type === "connected") {
              console.log("[NB SSE] 收到连接确认消息");
              setIsConnected(true);
              setIsConnecting(false);
              setError(null);
            ...
            // 检查是否为错误消息
            if (data.type === "error") {
              console.error("[NB SSE] 收到错误消息:", data.message);
            ...
            // 如果不是JSON，当作纯文本日志处理
            console.log(
              "[NB SSE] JSON解析失败，作为文本处理:",
              parseError instanceof Error
                ? parseError.message
                : String(parseError),
            );
            console.log("[NB SSE] 文本消息内容:", event.data);

            console.log("[NB SSE] 调用onMessage回调处理文本消息");
            onMessage({
              type: "log",
              message: event.data,
            });
            console.log("[NB SSE] 文本消息处理完成");
          }

          console.log("[NB SSE] ========== 消息处理结束 ==========");
        };
        ...
        const handleEvent = (event: MessageEvent) => {
          console.log("[NB SSE] ==== 自定义事件 ====", event.type);
          console.log("[NB SSE] 自定义事件数据:", event.data);
          processEvent(event);
        };
        ...
        customEventTypes.forEach((evt) => {
          console.log("[NB SSE] 注册自定义事件监听器:", evt);
          try {
            eventSource.addEventListener(evt, handleEvent as EventListener);
            eventListenersRef.current.set(evt, handleEvent);
          } catch (error) {
            console.warn(`[NB SSE] 注册事件监听器失败: ${evt}`, error);
          }
        });

        // 错误事件处理
        eventSource.onerror = (error) => {
          console.error("[NB SSE] 连接错误:", error);
          setIsConnecting(false);
          setIsConnected(false);
          setError("连接失败");

          // 手动模式，直接关闭连接
          console.log("[NB SSE] 手动模式，关闭连接");
          cleanup();
        ...
        console.error("[NB SSE] 创建连接失败:", error);
        setIsConnecting(false);
        setIsConnected(false);
